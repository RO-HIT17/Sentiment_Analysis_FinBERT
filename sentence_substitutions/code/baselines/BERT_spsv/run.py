...
class BertModel_dropout(BertPreTrainedModel):
    ...
    def forward(
        ...
        if not vanilla_bert:
            ...
            dropout_mask_thistoken = torch.cat((left_ones, dropout_mask, right_ones), 1).cpu()
...
def eval(test_data='C:\Rohit\Projects\Fintech\sentiment_analysis_for_business\sentence_recommendor\SmartWordSuggestions\data\sws\sws_test.json'):
    ...
    for sentidx, v in tqdm(js.items()):
        ...
        masked_ids = torch.tensor([tokenizer.convert_tokens_to_ids(tokens)]).cpu()
        segment_ids = torch.tensor([[0]*len(tokens)]).cpu()
