...
def suggest_replacements(sentence, top_k=3, exclude_punctuation=True):
    """
    ...
    Args:
        ...
        exclude_punctuation (bool): Whether to exclude punctuation from suggestions.
    ...
    for i in range(len(tokens)):
        ...
        # Exclude the original token and optional punctuation
        original_token = tokens[i]
        filtered_suggestions = [
            token for token in top_k_tokens
            if token != original_token and (not exclude_punctuation or token.isalnum())
        ]
...
if __name__ == "__main__":
    ...
    input_sentence = "The stock market is performing exceptionally well today."
    
    print(f"Input Sentence: {input_sentence}")
    suggestions = suggest_replacements(input_sentence, top_k=5)
